...
description: This skill should be used when user asks about "project structure", a "crewai project", "directory layout", placement of config or tools, or maintainable repository organization standards. It provides guidance for structuring source code, configuration, tests, outputs, and orchestration assets across crew and flow projects. Use it when teams need consistent file boundaries, clearer ownership, or scalable layouts for growth and onboarding. It helps enforce predictable structure conventions that improve discoverability, reduce integration friction, and keep maintenance workflows reliable across evolving CrewAI implementations.
version: 1.0.0
---
...
Define a practical operating model for project layout standards and maintainable repository organization in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent directory layouts, file placement rules, and structural conventions.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "project structure" outcomes with repeatable delivery quality.
- Use this skill when work requires "crewai project" outcomes with repeatable delivery quality.
- Use this skill when work requires "directory layout" outcomes with repeatable delivery quality.
- Use this skill when work requires "structure best practices" outcomes with repeatable delivery quality.
- Use this skill when work requires "file organization" outcomes with repeatable delivery quality.
- Use this skill when work requires "starter layout" outcomes with repeatable delivery quality.
...
- **Source Boundary**: Keep executable logic in predictable source paths to simplify tooling.
- **Config Boundary**: Store declarative configuration in dedicated config locations.
- **Tool Boundary**: Isolate custom tools for reuse and permission review.
- **Test Boundary**: Mirror source responsibilities with focused test modules.
- **Output Boundary**: Separate generated artifacts from tracked source code.
- **Crew Layout**: Organize crew projects around agents, tasks, and orchestration entry points.
- **Flow Layout**: Organize flow projects around stateful orchestration modules and crews.
- **Naming Rules**: Use consistent naming so imports, references, and ownership remain clear.
- **Scalability Path**: Prepare structure for additional crews, tools, and environments.
- **Documentation Surface**: Expose structure intent through concise README and reference files.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for project layout standards and maintainable repository organization and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-project-structure workstreams.
...
- [ ] Confirm scope statements align with the intended project layout standards and maintainable repository organization objective.
