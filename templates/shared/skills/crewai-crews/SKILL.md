---
name: crewai-crews
description: This skill should be used when user asks to "create a crew", "orchestrate agents", choose a "sequential process", choose a "hierarchical process", configure kickoff behavior, or standardize crew-level reliability controls. It provides guidance for composing agents and tasks into coherent execution topologies, selecting process strategy by complexity, and managing outputs, memory, caching, and observability. Use it when collaboration boundaries are unclear, manager logic is required, or multi-agent execution needs stronger governance to reduce drift, improve replayability, and maintain consistent outcome quality.
version: 1.0.0
---
...
Define a practical operating model for multi-agent orchestration, process selection, and execution control in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent crew composition, process settings, execution policies, and output handling conventions.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "create a crew" outcomes with repeatable delivery quality.
- Use this skill when work requires "orchestrate agents" outcomes with repeatable delivery quality.
...
- Use this skill when work requires "crew kickoff" outcomes with repeatable delivery quality.
- Use this skill when work requires "crew configuration" outcomes with repeatable delivery quality.
...
- **Crew Topology**: Define participating agents and their interaction boundaries for coherent collaboration.
- **Process Strategy**: Select sequential or hierarchical flow based on coordination complexity and control needs.
- **Manager Layer**: Introduce manager logic only when dynamic planning materially improves quality.
- **Output Envelope**: Normalize final outputs and task outputs for reliable post-processing and storage.
- **Execution Mode**: Choose sync, async, batch, or streaming according to latency and throughput goals.
- **Memory Policy**: Enable memory deliberately and monitor long-run context effects on relevance.
- **Caching Policy**: Cache stable tool responses to reduce repeated external requests and runtime cost.
- **Replayability**: Use deterministic naming and logging to support replay and incident analysis.
- **Safety Controls**: Set explicit iteration and delegation limits to avoid expensive drift.
- **Metrics Loop**: Track token usage and runtime metrics to support continuous optimization.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for multi-agent orchestration, process selection, and execution control and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-crews workstreams.
...
- [ ] Confirm scope statements align with the intended multi-agent orchestration, process selection, and execution control objective.
