...
description: This skill should be used when user asks to "migrate crew", "refactor crew", "upgrade to new version", move from crew to flow architecture, convert code to YAML, or modularize monolithic projects. It provides guidance for migration planning, risk control, parity validation, and staged rollout with rollback readiness. Use it when existing structure blocks scaling, compatibility upgrades are required, or refactoring must preserve production behavior during transition. It helps teams execute structural change safely through explicit checkpoints, dependency audits, and synchronized documentation updates.
version: 1.0.0
---
...
Define a practical operating model for architecture migration and structural refactoring in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent migration plans, transformed project structures, and verification checklists.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "migrate crew" outcomes with repeatable delivery quality.
- Use this skill when work requires "refactor crew" outcomes with repeatable delivery quality.
- Use this skill when work requires "upgrade to new version" outcomes with repeatable delivery quality.
- Use this skill when work requires "crew to flow" outcomes with repeatable delivery quality.
- Use this skill when work requires "code to YAML" outcomes with repeatable delivery quality.
- Use this skill when work requires "modularize project" outcomes with repeatable delivery quality.
...
- **Migration Scope**: Define boundaries, risks, and acceptance criteria before editing project structure.
- **State Preservation**: Protect current behavior through backups and reproducible checkpoints.
- **Crew-to-Flow**: Separate orchestration from execution by introducing explicit flow control.
- **Code-to-YAML**: Externalize static configuration for maintainability and faster iteration.
- **Modularization**: Split monolithic components by responsibility and dependency boundaries.
- **Version Upgrade**: Resolve compatibility concerns in controlled increments with rollback paths.
- **Validation Gate**: Verify parity after each migration stage before proceeding.
- **Dependency Audit**: Review external integrations before and after structural changes.
- **Documentation Sync**: Update operating docs and runbooks alongside code moves.
- **Risk Register**: Track migration risks and mitigation status through completion.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for architecture migration and structural refactoring and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-migration workstreams.
...
- [ ] Confirm scope statements align with the intended architecture migration and structural refactoring objective.
