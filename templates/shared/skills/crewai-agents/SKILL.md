---
name: crewai-agents
description: This skill should be used when user asks to "create an agent", "configure agent attributes", "set agent role", define an "agent goal", craft an "agent backstory", tune delegation or iteration limits, or select tools and models for a CrewAI agent. It provides structured guidance for designing agent contracts, aligning behavior to outcome criteria, and preventing drift through clear boundaries, observability, and validation checkpoints. Use it when agent behavior is inconsistent, responsibilities overlap, or a new role requires reliable operational standards before integration into a crew.
version: 1.0.0
---
...
Define a practical operating model for agent definition and behavioral configuration in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent agent role statements, goals, backstories, capability settings, and operating constraints.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "create an agent" outcomes with repeatable delivery quality.
- Use this skill when work requires "configure agent" outcomes with repeatable delivery quality.
- Use this skill when work requires "set agent role" outcomes with repeatable delivery quality.
- Use this skill when work requires "agent goal" outcomes with repeatable delivery quality.
- Use this skill when work requires "agent backstory" outcomes with repeatable delivery quality.
- Use this skill when work requires "agent best practices" outcomes with repeatable delivery quality.
...
- **Role Contract**: Define specialized responsibility boundaries so each agent contributes distinct expertise without overlap.
- **Goal Contract**: Specify outcome intent with clear completion signals to keep reasoning aligned during execution.
- **Backstory Context**: Provide domain context that shapes tone, assumptions, and decision priorities during responses.
- **Tool Boundary**: Assign only required tools and deny unnecessary capabilities to reduce failure surfaces.
- **Delegation Policy**: Control handoff behavior so collaboration occurs intentionally instead of uncontrolled bouncing.
- **Iteration Budget**: Set limits on retries and reflection loops to prevent runaway compute cost.
- **Model Fit**: Match model capability to task complexity before tuning other parameters.
- **Memory Scope**: Enable memory only when historical context improves outcomes for repeated workflows.
- **Reasoning Mode**: Use explicit planning mode when tasks require decomposition and risk-aware sequencing.
- **Observability**: Collect verbose traces only where needed to preserve signal while limiting noise.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for agent definition and behavioral configuration and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-agents workstreams.
...
- [ ] Confirm scope statements align with the intended agent definition and behavioral configuration objective.
