...
description: This skill should be used when user asks to "track tasks", "manage subtasks", review "task dependencies", monitor "feature progress", or operate the task-management CLI for structured delivery workflows. It provides guidance for maintaining accurate task ledgers, selecting eligible work, recording completion summaries, and validating dependency integrity. Use it when implementation plans are large, sequencing is complex, or blocked work needs transparent reporting. It helps teams coordinate execution with consistent status transitions, parallelization awareness, and dependable progress visibility across feature lifecycles.
version: 1.0.0
---
...
Define a practical operating model for feature task tracking, dependency control, and progress validation in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent task trees, status transitions, dependency maps, and completion records.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "track tasks" outcomes with repeatable delivery quality.
- Use this skill when work requires "manage subtasks" outcomes with repeatable delivery quality.
- Use this skill when work requires "task dependencies" outcomes with repeatable delivery quality.
- Use this skill when work requires "feature progress" outcomes with repeatable delivery quality.
- Use this skill when work requires "task CLI" outcomes with repeatable delivery quality.
- Use this skill when work requires "status workflow" outcomes with repeatable delivery quality.
...
- **Feature Ledger**: Store one canonical task ledger for each feature implementation effort.
- **Subtask Contract**: Represent each subtask with clear status, dependencies, and exit criteria.
- **Dependency Graph**: Resolve prerequisites before execution to avoid blocked rework.
- **Eligibility Check**: Select only tasks whose dependencies are complete and validated.
- **Parallel Window**: Run independent eligible subtasks concurrently to increase throughput.
- **Completion Record**: Capture concise summaries when marking subtasks complete.
- **Validation Pass**: Run integrity checks to detect schema and dependency defects.
- **Progress Signal**: Use status views to communicate progress and bottlenecks clearly.
- **Integration Hook**: Coordinate CLI usage with planning agents and delivery workflows.
- **Governance Rule**: Enforce consistent transitions so reports remain trustworthy.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for feature task tracking, dependency control, and progress validation and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating task-management workstreams.
...
- [ ] Confirm scope statements align with the intended feature task tracking, dependency control, and progress validation objective.
