---
name: crewai-flows
description: This skill should be used when user asks to "create a flow", "orchestrate crews", implement "state management", build "routing logic", apply "flow decorators", or design persistent event-driven workflows. It provides guidance for structuring start, listen, and router nodes; managing typed state transitions; and coordinating parallel branches with safe merge behavior. Use it when orchestration complexity exceeds a simple crew, when recovery and resumability are required, or when execution paths need explicit control, human checkpoints, and traceable decision logic for reliable long-running automation.
version: 1.0.0
---
...
Define a practical operating model for event-driven workflow orchestration with state, routing, and persistence in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent flow states, decorator wiring, routing branches, persistence decisions, and orchestration checkpoints.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "create a flow" outcomes with repeatable delivery quality.
- Use this skill when work requires "orchestrate crews" outcomes with repeatable delivery quality.
- Use this skill when work requires "state management" outcomes with repeatable delivery quality.
- Use this skill when work requires "routing logic" outcomes with repeatable delivery quality.
- Use this skill when work requires "flow decorators" outcomes with repeatable delivery quality.
- Use this skill when work requires "flow architecture" outcomes with repeatable delivery quality.
...
- **Start Node**: Define explicit entry points that initialize state and trigger deterministic execution paths.
- **Listen Node**: Subscribe downstream logic to prior outputs without coupling unrelated concerns.
- **Router Node**: Branch execution by returning stable route keys from explicit decision logic.
- **State Model**: Use structured state fields so each step reads and writes predictable values.
- **Parallel Fan-out**: Run independent branches concurrently when no data dependency exists.
- **Join Strategy**: Aggregate branch outputs with clear merge rules and conflict handling.
- **Persistence Layer**: Persist long-running state for resumability and operational recovery.
- **Human Checkpoint**: Pause and request review at risk boundaries or policy-sensitive transitions.
- **Observability**: Instrument transitions and route decisions for troubleshooting and audits.
- **Flow Hygiene**: Keep nodes focused and composable to prevent monolithic flow classes.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for event-driven workflow orchestration with state, routing, and persistence and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-flows workstreams.
...
- [ ] Confirm scope statements align with the intended event-driven workflow orchestration with state, routing, and persistence objective.
