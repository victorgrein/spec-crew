---
name: crewai-memory
description: This skill should be used when user asks to "configure memory", enable "agent memory", design "short-term memory", design "long-term memory", implement entity recall, or select embedder and persistence policies. It provides guidance for deciding memory scope, tuning retrieval precision, and managing retention and privacy boundaries in CrewAI systems. Use it when repeated workflows require historical context, when recall quality is inconsistent, or when stale context degrades decisions. It helps establish reliable memory architecture that improves continuity while preserving control, traceability, and operational correctness.
version: 1.0.0
---
...
Define a practical operating model for memory architecture, persistence decisions, and recall quality control in CrewAI implementations.
Organize decisions, guardrails, and review criteria so teams produce consistent memory enablement settings, embedder selection, retention policies, and recall patterns.
Reduce rework by separating fast core guidance from deeper reference and example materials.

## When to Use This Skill

- Use this skill when work requires "configure memory" outcomes with repeatable delivery quality.
- Use this skill when work requires "agent memory" outcomes with repeatable delivery quality.
- Use this skill when work requires "short-term memory" outcomes with repeatable delivery quality.
- Use this skill when work requires "long-term memory" outcomes with repeatable delivery quality.
- Use this skill when work requires "entity memory" outcomes with repeatable delivery quality.
- Use this skill when work requires "memory persistence" outcomes with repeatable delivery quality.
...
- **Short-term Memory**: Capture in-session context to improve continuity within a single run.
- **Long-term Memory**: Persist learned information across runs for cumulative workflow quality.
- **Entity Memory**: Track facts about people, systems, or concepts for targeted recall.
- **Embedder Fit**: Select embedding providers that align with language, scale, and cost needs.
- **Retrieval Precision**: Tune retrieval scope to avoid irrelevant memory injection into prompts.
- **Retention Policy**: Define what to keep, expire, or archive to control growth.
- **Privacy Boundary**: Store only approved data and enforce access controls for sensitive content.
- **Memory Scope**: Enable memory selectively so simple tasks remain lightweight and deterministic.
- **Evaluation Loop**: Validate memory usefulness through before-and-after quality comparisons.
- **Failure Handling**: Detect stale or conflicting memory and prioritize current trusted sources.

## Quick Start

1. Define the immediate objective and the final acceptance criteria before writing configuration details.
2. Capture scope boundaries for memory architecture, persistence decisions, and recall quality control and record assumptions that affect downstream decisions.
...
- Prioritize outcome clarity over implementation detail when initiating crewai-memory workstreams.
...
- [ ] Confirm scope statements align with the intended memory architecture, persistence decisions, and recall quality control objective.
