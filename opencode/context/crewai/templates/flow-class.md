...
Templates for creating CrewAI Flow classes.
...
class ParallelFlow(Flow[ParallelFlowState]):
    ...
    def research_topic_b(self, topics):
        # Research second topic (runs in parallel)
...
The recommended way to run flows is using the CrewAI CLI:

```bash
crewai run
```

This requires `type = "flow"` in your `pyproject.toml`.
```
